...
loadDemoData c = do icons <- MaybeT $ createFont c "icons" (FileName "entypo.ttf")
                    normal <- MaybeT $ createFont c "sans" (FileName "Roboto-Regular.ttf")
                    bold <- MaybeT $ createFont c "sans-bold" (FileName "Roboto-Bold.ttf")
                    images <- loadImages
